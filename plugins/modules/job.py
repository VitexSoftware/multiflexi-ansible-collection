...
description:
    - This module allows you to create, update, get, list and delete jobs in Multiflexi using the multiflexi-cli tool with idempotency logic.
...
options:
    ...
    app_id:
        description:
            - The ID of the application (used for lookup or update).
        required: false
        type: int
    runtemplate_id:
        description:
...
def run_cli(module, args):
    ...
    # Always use --format json for all commands
...
def find_existing_job(module):
    ...
    if module.params.get('job_id'):
        try:
            ...
            return None
    elif module.params.get('runtemplate_id') and module.params.get('scheduled'):
        # Try to find by unique combination
        try:
            jobs = run_cli(module, ['job:list'])
            for job in jobs if isinstance(jobs, list) else []:
                if (str(job.get('runtemplate_id')) == str(module.params['runtemplate_id']) and
                    str(job.get('scheduled')) == str(module.params['scheduled'])):
                    if module.params.get('app_id'):
                        if str(job.get('app_id')) != str(module.params['app_id']):
                            continue
                    return job
        except Exception:
            return None
...
def run_module():
    module_args = dict(
        state=dict(type='str', required=True, choices=['present', 'get', 'absent']),
    ...
    if state == 'get':
        ...
        if job:
            ...
            for field in ['app_id', 'runtemplate_id', 'scheduled', 'executor', 'schedule_type']:
                ...
                if val is not None and str(val) != str(job.get(field)):
            ...
            # job:create only supports runtemplate_id, scheduled, executor, schedule_type
            for field in ['runtemplate_id', 'scheduled', 'executor', 'schedule_type']:
                val = module.params.get(field)
                if val is not None:
        ...
        if job:
            if module.check_mode:
                result['changed'] = True
                module.exit_json(**result)
            run_cli(module, ['job:delete', '--id', str(job['id'])])
            result['changed'] = True
        else:
